...
describe('Command Availability Smoke Tests', () => {
  ...
  describe('Platform-Specific Files', () => {
    ...
    test('copilot has chatmode file', async () => {
      expect(await fileExists('copilot/chatmodes/Code Captain.chatmode.md')).toBe(true)
    })

    test('windsurf has rules file', async () => {
      expect(await fileExists('windsurf/rules/cc.md')).toBe(true)
    })
  })
  ...
  describe('Documentation Files', () => {
    test.each(['cursor', 'copilot'])('platform %s has best practices documentation', async (platform) => {
      expect(await fileExists(`${platform}/docs/best-practices.md`)).toBe(true)
    })
  })
  ...
  describe('Command Coverage Analysis', () => {
    test('report command coverage across platforms', async () => {
      ...
      expect(coverage.windsurf.coreCommands.length).toBeGreaterThanOrEqual(6)
    })
  })
})