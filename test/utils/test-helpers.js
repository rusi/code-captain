...
export const PLATFORM_CONFIG = {
  ...
  windsurf: {
    ...
    extension: '.md',
    prefix: '/',
    requiredFrontmatter: { description: 'string' }
  },
  claude: {
...
export function validateFrontmatter(data, platform) {
  ...
  for (const [key, expectedType] of Object.entries(config.requiredFrontmatter)) {
    if (!(key in data)) {
    ...
    if (Array.isArray(expectedType) && !expectedType.includes(data[key])) {
      throw new Error(`Field ${key} must be one of: ${expectedType.join(', ')}`)
    }
  }
