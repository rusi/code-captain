 ...
 * Scans cursor, copilot, windsurf, and claude-code command files.
 */
...
class CommandAnalyzer {
    ...
    async analyzePlatform(platform) {
        ...
        if (!fs.existsSync(fullPath)) {
            console.log(`⚠️  Platform ${platform.name} path not found: ${platform.path}`);
            return;
        }
        ...
        for (const file of files) {
            ...
            this.results.platforms[platform.name].totals.tokens.characters += fileAnalysis.tokens.characters;
            this.results.platforms[platform.name].totals.tokens.words += fileAnalysis.tokens.words;
            this.results.platforms[platform.name].totals.tokens.enhanced += fileAnalysis.tokens.enhanced;
        }
    }
    ...
    generateSummary() {
        ...
        for (const [platformName, platform] of Object.entries(this.results.platforms)) {
            ...
            this.results.summary.push({
                ...
                avgTokensPerFile: platform.totals.files > 0 ?
                    Math.round(platform.totals.tokens.enhanced / platform.totals.files) : 0
            });
        }
    ...
    displayResults() {
        ...
        console.log(sprintf('%-20s %8s %12s %12s %12s',
            'Platform', 'Files', 'Characters', 'Tokens', 'Avg/File'));
        console.log('-'.repeat(70));

        this.results.summary
            .sort((a, b) => b.tokens - a.tokens)
            .forEach(platform => {
                console.log(sprintf('%-20s %8d %12s %12s %12d',
                    platform.platform,
                    platform.files,
        ...
        // Grand totals
        console.log('-'.repeat(70));
        console.log(sprintf('%-20s %8d %12s %12s %12s',
            'TOTAL',
            ...
            Math.round(this.results.totals.approximateTokens / this.results.totals.files)
        ));
        ...
        topPlatforms.forEach(platformSummary => {
            ...
            platform.files
                ...
                .forEach(file => {
                    console.log(sprintf('  %-30s %8s tokens (%s chars)',
                        file.name,
        ...
        console.log('-'.repeat(50));
        console.log(`Total tokens: ~${this.formatNumber(this.results.totals.approximateTokens)}`);
        console.log(`OpenAI GPT-4: ~$${(this.results.totals.approximateTokens * 0.00003).toFixed(2)}`);
        console.log(`Anthropic Claude: ~$${(this.results.totals.approximateTokens * 0.000015).toFixed(2)}`);
        ...
        if (this.results.summary.some(p => p.avgTokensPerFile > 2000)) {
            console.log('• Some commands are quite large - consider breaking into smaller components');
        }
        console.log('• These approximations are suitable for planning and comparison purposes');
        console.log('• For production billing, use actual tokenizer APIs when available');
    }
...
**Estimated Tokens:** ${this.formatNumber(this.results.totals.approximateTokens)}  
...
**Key Metrics:**
- **${this.results.totals.files} total command files** across ${Object.keys(this.results.platforms).length} platforms
- **~${this.formatNumber(this.results.totals.approximateTokens)} estimated tokens** total
- **${Math.round(this.results.totals.approximateTokens / this.results.totals.files)} average tokens per file**

**Cost Estimates:**
- OpenAI GPT-4: ~$${(this.results.totals.approximateTokens * 0.00003).toFixed(2)}
- Anthropic Claude: ~$${(this.results.totals.approximateTokens * 0.000015).toFixed(2)}
...
`;
        ...
        this.results.summary
            .sort((a, b) => b.tokens - a.tokens)
            .forEach(platform => {
                markdown += `| ${platform.platform} | ${platform.files} | ${this.formatNumber(platform.characters)} | ${this.formatNumber(platform.tokens)} | ${platform.avgTokensPerFile} |\n`;
            });

        markdown += `| **TOTAL** | **${this.results.totals.files}** | **${this.formatNumber(this.results.totals.characters)}** | **${this.formatNumber(this.results.totals.approximateTokens)}** | **${Math.round(this.results.totals.approximateTokens / this.results.totals.files)}** |
...
`;
        ...
        const sortedPlatforms = this.results.summary.sort((a, b) => b.tokens - a.tokens);

        sortedPlatforms.forEach(platformSummary => {
            const platform = this.results.platforms[platformSummary.platform];
            markdown += `### ${platformSummary.platform.charAt(0).toUpperCase() + platformSummary.platform.slice(1)}
...
`;
            ...
            platform.files
                .sort((a, b) => b.tokens.enhanced - a.tokens.enhanced)
                .forEach(file => {
                    markdown += `| \`${file.name}\` | ${this.formatNumber(file.size)} | ${this.formatNumber(file.tokens.enhanced)} |\n`;
                });
...
`;
        ...
        this.results.summary
            ...
            .forEach(platform => {
                ...
                markdown += `**${platform.platform.padEnd(18)}** \`${bar}\` ${this.formatNumber(platform.tokens)} tokens\n`;
            });
...
`;
        ...
        this.results.summary
            ...
            .forEach(platform => {
                ...
                markdown += `| ${platform.platform} | ${this.formatNumber(platform.tokens)} | $${gpt4Cost} | $${claudeCost} |\n`;
            });
...
`;
        ...
        allFiles
            ...
            .slice(0, 10)
            .forEach((file, index) => {
                markdown += `| ${index + 1} | \`${file.name}\` | ${file.platform} | ${this.formatNumber(file.tokens)} |\n`;
            });
...
- 💰 **Monitor costs** - With ${this.formatNumber(this.results.totals.approximateTokens)} tokens, usage costs should be tracked
`;
        }
...
**Platforms Analyzed:**
${Object.entries(this.results.platforms).map(([name, platform]) => `- **${name}**: \`${platform.path}\``).join('\n')}

**Files Scanned:** ${this.results.totals.files} total markdown files across all platforms
...
async function main() {
    try {
        ...
        const projectRoot = path.dirname(__dirname);
        const outputFile = path.join(projectRoot, '.code-captain', 'docs', 'token-analysis.md');
